...
if [[ $1 == "forward" ]];then
    ...
    iptables -t nat -I PREROUTING 1 -i "$AP_IF" -d "$AP_NET" -j RETURN
    iptables -t nat -I PREROUTING 1 -i "$AP_IF" -d "$USB_NET" -j RETURN
    ...
    sudo iptables -t nat -D PREROUTING -i "$AP_IF" -d "$AP_NET" -j RETURN 2>/dev/null || true
    sudo iptables -t nat -D PREROUTING -i "$AP_IF" -d "$USB_NET" -j RETURN 2>/dev/null || true
